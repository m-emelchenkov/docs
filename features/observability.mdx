...
For approved actions, see [Human Approval](/features/human-approval). For custom audit shapes, use post-hooks (below).
...
`run_in_background=True` keeps the audit write off the response path. See [Hooks](/features/human-approval#post-hooks-output) for the full pattern.
...
[Security & Auth →](/features/security-and-auth)