...
| `enable_search_messages` | `bool` | `False` | Enable `search_messages` tool. Requires the `search:read` OAuth scope. |
...
| `search_messages(query, limit=20)` | Search messages across the workspace. |
