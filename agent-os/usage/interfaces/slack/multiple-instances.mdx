...
# Each Slack interface gets its own credentials and prefix
agent_os = AgentOS(
...
<Steps>
  ...
  <Step title="Set Environment Variables">
    ```bash
    export RESEARCH_SLACK_TOKEN=xoxb-research-bot-token
    export RESEARCH_SLACK_SIGNING_SECRET=research-signing-secret
    export ANALYST_SLACK_TOKEN=xoxb-analyst-bot-token
    export ANALYST_SLACK_SIGNING_SECRET=analyst-signing-secret
...
- **Separate Bot Tokens**: Each interface uses its own Slack app (each app needs its own token and signing secret)
