...
Once it works, keep developing with the temporary token and test number, or move toward production by adding and verifying your own business number and generating a permanent token.

To create a permanent token:
1. Go to [Meta Business Settings](https://business.facebook.com/settings) > **System Users**.
2. Add an admin-level system user.
3. Assign your app and WhatsApp Business Account with **Full control**.
4. Click **Generate Token** and select `whatsapp_business_messaging` and `whatsapp_business_management`.
5. Store the token securely. It does not expire unless revoked.
