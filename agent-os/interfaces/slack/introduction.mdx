...
<AccordionGroup>
  ...
  <Accordion title="Bot responds to its own messages">
    **Cause:** Missing bot self-loop filter.

    **Fix:** This is handled automatically by filtering `bot_id` and bot lifecycle subtypes. If it still happens, verify your Slack app's bot user has a `bot_id` in events.
  </Accordion>
