...
Register at least one source in `content_sources` on your Knowledge instance. See [Cloud Storage Sources](/knowledge/concepts/cloud-storage) for the provider classes (`S3Config`, `GcsConfig`, `SharePointConfig`, `GitHubConfig`, `AzureBlobConfig`) and their parameters.
...
`config_id` and `path` are required. See the [upload-remote-content reference](/reference-api/schema/knowledge/upload-remote-content) for all form fields and query parameters. Valid `reader_id` and `chunker` values come from [`/knowledge/config`](/reference-api/schema/knowledge/get-config) under `readers` and `chunkers`.
