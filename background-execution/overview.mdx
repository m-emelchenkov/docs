...
description: "Decouple execution from the client lifecycle with background runs and SSE reconnection."
mode: wide
---

Run agents, teams, and workflows in the background by passing `background=True` to `.arun()`. Execution continues even if the client disconnects. Two patterns are available depending on whether you also set `stream=True`.
...
async def main():
    ...
    run_output = await agent.arun(
        "Write a detailed analysis of quantum computing trends.",
        background=True,
    )
...
async def main():
    run_output = await team.arun(
        "Research AI trends and related stocks.",
        background=True,
    )
    print(f"Run ID: {run_output.run_id}, Status: {run_output.status}")
...
Combine `background=True` with `stream=True` for resumable SSE streaming. The run executes in a detached `asyncio.Task` that survives client disconnections. Events are buffered with sequential `event_index` values so clients can reconnect without losing events.
...
Resumable streaming requires an [AgentOS server](/agent-os/run-your-os) running. Pass `background=true` and `stream=true` in the request. The pattern is the same for agents, teams, and workflows. Only the URL path differs.

<Note>
For workflows, SSE is the default reconnection transport. Workflows also support WebSocket-based reconnection. See the [WebSocket reconnect example](/examples/workflows/advanced-concepts/long-running/websocket-reconnect).
</Note>
...
If the client disconnects (page refresh, network loss), reconnect using the `/resume` endpoint with the last received `event_index`:
