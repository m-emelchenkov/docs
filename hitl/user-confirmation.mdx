...
<Tip>
When using `continue_run()` after a confirmation pause, the agent's conversation history is fully preserved. This means any context from previous messages (like user introductions or earlier tool results) remains available to the agent when it resumes execution.
</Tip>
...
When using user confirmation with agents that have `add_history_to_context=True`, the conversation history is automatically preserved across confirmation pauses. This ensures the agent maintains full context when resuming execution:

```python
agent = Agent(
    model=OpenAIResponses(id="gpt-5.2"),
    tools=[sensitive_operation],
    add_history_to_context=True,  # Enables history preservation
)

# First interaction
run_response = agent.run("Hi, I'm John. I need help with a sensitive operation.")

# The agent will pause when it tries to use the sensitive_operation tool
if run_response.is_paused:
    ...
    # Continue the run - the agent remembers that the user is John
    response = agent.continue_run(run_id=run_response.run_id, requirements=run_response.requirements)
    # Agent's response will maintain context: "Hi John, I've completed the sensitive operation..."
```
