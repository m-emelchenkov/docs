...
HITL works the same way for Teams. When a member agent's tool requires confirmation, user input, or external execution, the team run pauses until the requirement is resolved. The requirement includes `member_agent_name` so you know which member triggered it.
...
Tools attached directly to the Team (not member agents) also support HITL. When the team leader calls a tool requiring confirmation, the run pauses the same way.
