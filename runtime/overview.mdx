...
The runtime is one of three layers of an [agent platform](/agent-platform/overview): the SDK to build, the runtime to run, and the control plane to manage. To ship agents to real users, you need to run them as a horizontally scalable, durable service reachable over an API. That's what the runtime gives you.
