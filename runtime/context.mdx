...
1. **Service Account + domain-wide delegation** (headless, for workspace accounts):
...
2. **OAuth** (interactive, for personal accounts):
   - Set `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `GOOGLE_PROJECT_ID`
   - Opens browser on first use, caches token locally
...
Read sub-agent gets: `search_emails`, `get_message`, `list_labels`.
Write sub-agent gets: `draft_email`, `send_email`, `reply_to_email`, `create_label`, `apply_label`.
...
Read sub-agent gets: `list_events`, `get_event`, `list_calendars`.
Write sub-agent gets: `create_event`, `update_event`, `delete_event`.
...
# Pass through to underlying providers
agent = Agent(
    model=...,
...
This is the recommended approach for personal assistant agents that need full Google Workspace access.

### WikiContextProvider

Read/write access to a directory of markdown files with two backend options:

```python
from agno.context.wiki import WikiContextProvider
from agno.context.wiki.backend import FileSystemBackend, GitBackend

# Local filesystem (no versioning)
wiki = WikiContextProvider(
    backend=FileSystemBackend(root="./docs"),
    id="wiki",
)

# Git-backed (auto-commit, rebase, push)
wiki = WikiContextProvider(
    backend=GitBackend(
...
The `GitBackend` runs `commit_after_write` after every update, optionally pushing to remote. This turns any markdown directory into a version-controlled wiki the agent can edit.

Optional web ingestion: pass a `web` backend to let the write sub-agent pull content from URLs before writing:
...
| [`06_mcp_server.py`](https://github.com/agno-agi/agno/blob/main/cookbook/12_context/06_mcp_server.py) | MCPContextProvider with `mode=ContextMode.tools` |
...
| [`13_workspace.py`](https://github.com/agno-agi/agno/blob/main/cookbook/12_context/13_workspace.py) | WorkspaceContextProvider for local directories |
| [`14_wiki_filesystem.py`](https://github.com/agno-agi/agno/blob/main/cookbook/12_context/14_wiki_filesystem.py) | WikiContextProvider with filesystem backend |
| [`15_wiki_git.py`](https://github.com/agno-agi/agno/blob/main/cookbook/12_context/15_wiki_git.py) | WikiContextProvider with git backend (auto-commit) |
| [`18_gmail.py`](https://github.com/agno-agi/agno/blob/main/cookbook/12_context/18_gmail.py) | GmailContextProvider: search, read, draft, send |
| [`19_calendar.py`](https://github.com/agno-agi/agno/blob/main/cookbook/12_context/19_calendar.py) | GoogleCalendarContextProvider: events and availability |
| [`20_google_workspace.py`](https://github.com/agno-agi/agno/blob/main/cookbook/12_context/20_google_workspace.py) | Full Google Workspace: Drive + Gmail + Calendar |
