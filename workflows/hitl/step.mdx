...
Steps support three HITL modes: confirmation (approve/reject before execution), user input (collect parameters before execution), and output review (review results after execution).
...
Pause after a step executes so the user can review the output before the workflow continues. If the user rejects, the step re-executes (up to `max_retries`).
...
while run_output.is_paused:
    for req in run_output.steps_requiring_confirmation:
        print(f"Output: {req.step_output}")
        print(f"Message: {req.confirmation_message}")
...
Set a deadline for the user to respond. If it expires, `on_timeout` fires automatically.
