...
| [Output Review](#output-review) | Review output after step execution | `requires_output_review=True` |
| [Iteration Review](#iteration-review) | Review each loop iteration | `requires_iteration_review=True` |
...
Pause after a step executes so the user can review the output. Supported on Step and Router.
...
When rejected with `on_reject=OnReject.retry`, the step re-executes up to `max_retries` times.
...
| `OnReject.retry` | Re-execute the step (for output review) |
| `OnReject.else_branch` | For Condition only: execute `else_steps` (default for Condition) |

## Timeout

Set a timeout for HITL pauses. If the user does not respond in time, `on_timeout` determines the action.
...
<CardGroup cols={3}>
  ...
  <Card title="Loop HITL" icon="rotate" href="/workflows/hitl/loop">
    Confirmation and iteration review on loops
  </Card>
