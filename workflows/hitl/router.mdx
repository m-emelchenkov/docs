...
title: Router HITL
sidebarTitle: Router
description: Let users select routes, confirm routing decisions, or review router output.
keywords: [workflow, router, HITL, route selection, confirmation, output review, human review]
---

Routers support three HITL modes: user selection (user chooses routes), confirmation (user approves automated routing), and output review (review results after execution).
...
Pause after the router's selected route executes so the user can review the output.

```python
Router(
    name="content_router",
    choices=[
        Step(name="draft", executor=draft_content),
        Step(name="summarize", executor=summarize_content),
    ],
    selector=select_route,
    human_review=HumanReview(
        requires_output_review=True,
        output_review_message="Review the router output.",
...
When rejected with `on_reject=OnReject.retry`, the selected route re-executes up to `max_retries` times.
...
Set a deadline for user responses during router HITL pauses.
