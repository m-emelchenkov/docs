...
keywords: [workflow, router, HITL, route selection, confirmation, output review, branching]
---
...
Review a router's output after execution. If rejected, the reviewer can pick a different route. Set `on_reject=OnReject.retry` to re-route on rejection.
...
| `hitl_max_retries` | `int` | `3` | Maximum re-route attempts |
