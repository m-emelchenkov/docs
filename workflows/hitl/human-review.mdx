...
`HumanReview` groups all HITL settings into a single object instead of passing them as separate parameters on Step, Loop, or Router.
...
# This raises ValueError: requires_output_review is not supported on Loop
Loop(
    name="my_loop",
    steps=[...],
