...
description: "Turn text, images, audio, video, and PDFs into typed Pydantic objects."
---

Define the schema, pass the input, get a validated object back. The model fills the schema; Pydantic enforces it.
...
Missing fields come back `null` instead of a hallucinated value because the instruction says so and the schema allows it.
...
|-------|----------|----------|
| Text | `agent.run(text)` | [text_extraction](https://github.com/agno-agi/agno/tree/main/cookbook/data_labeling/text_extraction) |
| Image | `images=[Image(url=...)]` | [image_extraction](https://github.com/agno-agi/agno/tree/main/cookbook/data_labeling/image_extraction) |
| Audio | `audio=[Audio(content=...)]` | [audio_extraction](https://github.com/agno-agi/agno/tree/main/cookbook/data_labeling/audio_extraction) |
| Video | `videos=[Video(content=...)]` | [video_extraction](https://github.com/agno-agi/agno/tree/main/cookbook/data_labeling/video_extraction) |
| PDF | `files=[File(url=...)]` | [document_extraction](https://github.com/agno-agi/agno/tree/main/cookbook/data_labeling/document_extraction) |

## Extract then embed

`image_extraction_to_vectordb` extends extraction with an embed-and-store step: describe each image into a typed object, flatten it to a searchable string, embed it, and store it in LanceDb for similarity search. See the [cookbook](https://github.com/agno-agi/agno/tree/main/cookbook/data_labeling/image_extraction_to_vectordb).
...
- [Text extraction cookbook](https://github.com/agno-agi/agno/tree/main/cookbook/data_labeling/text_extraction)
